...
function initSection6(libs: AnimationLibs) {
  const elements = getSection6Elements(libs);
  return elements;
}

export const createSection6: CreateTimelineSection<Section6Deps> = libs => {
  const elements = initSection6(libs);
  const animateText = createSequentialThresholdAnimations(libs, [
    { elements: elements.titleSplit.chars },
    { elements: elements.descriptionSplit.lines, overlap: 0.25, stagger: 0.1 },
