...
function getSection3Elements(libs: AnimationLibs) {
  ...
  if (!content) throw new Error('Section 4 content not found');
  if (!video) throw new Error('Section 4 video not found');
  if (!textContainer) throw new Error('Section 4 text container not found');
  if (!textItems) throw new Error('Section 4 text items not found');
  if (!textItemsArray) throw new Error('Section 4 text items array not found');
...
/**
 * Computes the y-offset for each text item that would center it
 * in the same position as the first item. Uses relative positions
 * so the result is independent of any current y-transform.
 */
...
/**
 * Steep S-curve that creates a snap/detent effect.
 * Spends more time near 0 and 1, transitions quickly in the middle.
 */
...
/**
 * Maps a linear progress value (0–1) to a snapped y-position
 * that centers each text item in sequence.
 */
...
export const createSection3: CreateTimelineSection<Section3Deps> = libs => {
      ...
      },
      {
        ...
        update: progress => {
          if (progress < 0.05) {
            ...
            if (isPlaying) return;
            if (typeof elements.video.play === 'function') {
              const p = elements.video.play();
